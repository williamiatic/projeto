...
<html lang="pt-br">
	...
	<body>
        ...
        $result = select("produto", "count(id_produto) as num_result", "where empresa_id = {$_SESSION['id']} and quantidade > '0'");
		...
		$max_links = 2;
		echo "<a href='listarprodutosestoquefront.php?pagina=1'>Primeira</a> ";
		
		for($pag_ant = $pagina - $max_links; $pag_ant <= $pagina - 1; $pag_ant++){
			if($pag_ant >= 1){
				echo "<a href='listarprodutosestoquefront.php?pagina=$pag_ant'>$pag_ant</a> ";
			}
		}
		...
		for($pag_dep = $pagina + 1; $pag_dep <= $pagina + $max_links; $pag_dep++){
			if($pag_dep <= $quantidade_pg){
				echo "<a href='listarprodutosestoquefront.php?pagina=$pag_dep'>$pag_dep</a> ";
			}
		}
		
        echo "<a href='listarprodutosestoquefront.php?pagina=$quantidade_pg'>Ultima</a>". "<br><hr>";
        
		$query = select("produto",
		"nm_fornecedor, id_produto, nm_produto, nm_categoria, marca, descricao, peso, controlado, quantidademinima, quantidade, lote, precounidade, precolote",
		"join fornecedor on id_fornecedor = fornecedor_id and fornecedor.empresa_id = {$_SESSION['id']}
		join categoria on id_categoria = categoria_id and categoria_id and quantidade > '0'", NULL, "LIMIT $inicio, $qnt_result_pg");
		...
		echo "<a href='listarprodutosestoquefront.php?pagina=$quantidade_pg'>Ultima</a>";
